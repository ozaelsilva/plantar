...
begin
  ...
  begin
    ...
    SysMensagem('Arquivo de Idioma não Encontrado!',dsErro);
end;

function SysMensagem(vrTexto: String; vrTipoMensagem : TipoMensagem) : Boolean;
begin
  { Função Para Controlar Mensagens }
  Result := false;
  case vrTipoMensagem of
    dsConfirmacao: begin
      if MessageBox(Application.Handle, Pchar (vrTexto), 'Confirmação', MB_OKCANCEL + MB_ICONQUESTION) = ID_OK then
        Result := true;
    end;
    dsAviso       : begin
    ...
    dsInformacao  : begin
      if MessageBox(Application.Handle, Pchar (vrTexto), 'Informação', MB_OK + MB_ICONINFORMATION) = ID_OK then
        Result := true;
    end;
    dsErro        : begin
